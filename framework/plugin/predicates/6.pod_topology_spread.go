...
func (pl PodTopologySpread) Debugg() {
	r.KETI_LOG_L2(fmt.Sprintf("[stage] F#5. %s", pl.Name()))
}
...
// calPreFilterState5 computes preFilterState5 describing how pods are spread on topologies.
func (pl *PodTopologySpread) calPreFilterState(pod *v1.Pod, cache *r.NodeCache) (*preFilterState5, error) {
...
func (pl PodTopologySpread) Filter(nodeInfoCache *r.NodeCache, newPod *r.QueuedPodInfo) {
	...
	if err != nil {
		//run prefilter in podtopologyspread error
	}

	for nodeName, nodeinfo := range nodeInfoCache.NodeInfoList {
		if !nodeinfo.PluginResult.IsFiltered {
			// However, "empty" preFilterState5 is legit which tolerates every toSchedule Pod.
			...
			for _, c := range s.Constraints {
				...
				if !ok {
					reason := fmt.Sprintf("node doesn't have required label : %d ", tpKey)
				...
				if err != nil {
					reason := fmt.Sprintf("%v Internal error occurred while retrieving value precalculated in PreFilter topologyKey %s paths %s", err, tpKey, s.TpKeyToCriticalPaths)
					filterState := r.FilterStatus{r.Error, pl.Name(), reason, nil}
					nodeinfo.PluginResult.FilterNode(nodeName, filterState)
					nodeInfoCache.NodeCountDown()
					break
				}
				...
				if skew > int(c.MaxSkew) {
					reason := fmt.Sprintf("node failed spreadConstraint: matchNum(%d)+selfMatchNum(%d)-minMatchNum(%d)>maxSkew(%d), topologyKey:%d, ", matchNum, selfMatchNum, minMatchNum, c.MaxSkew, tpKey)
					filterState := r.FilterStatus{r.UnschedulableAndUnresolvable, pl.Name(), reason, nil}
					nodeinfo.PluginResult.FilterNode(nodeName, filterState)
					nodeInfoCache.NodeCountDown()
					break
...
func (s *preFilterState5) minMatchNum(tpKey string, minDomains int32, enableMinDomainsInPodTopologySpread bool) (int, error) {
